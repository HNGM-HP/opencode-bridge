# 飞书 × OpenCode 桥接服务 v2.8.3 (Group)

[![v2.8.3](https://img.shields.io/badge/v2.8.3-3178C6)]()
...
这不是“换个文案”的小版本，而是一次架构换代。`v2.8.3` 将桥接核心从“单文件堆逻辑”重构为“平台适配层 + 根路由器 + OpenCode 事件中枢 + 领域处理器”的分层体系，重点解决跨平台扩展、权限闭环稳定性、目录实例一致性和线上可维护性。
...
| 维度 | OpenClaw / 同类桥接常见形态 | 本项目（v2.8.3） |
|---|---|---|
...
> **注意**: 本章节适用于 v2.8.3 版本，涉及路由器模式的灰度升级流程。
