import * as path from 'path';
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';
import { P2PHandler } from '../src/handlers/p2p.js';
import { opencodeClient } from '../src/opencode/client.js';
import { chatSessionStore } from '../src/store/chat-session.js';
import type { CreateChatCardData } from '../src/feishu/cards.js';

type InternalP2PHandler = {
  buildCreateChatCardData: (selectedSessionId?: string) => Promise<CreateChatCardData>;
};

describe('P2P create_chat 工作项目来源', () => {
  const originalAllowedDirectories = process.env.ALLOWED_DIRECTORIES;
  const originalDefaultWorkDirectory = process.env.DEFAULT_WORK_DIRECTORY;
  const originalEnableManualSessionBind = process.env.ENABLE_MANUAL_SESSION_BIND;
  const originalProjectAliases = process.env.PROJECT_ALIASES;

  beforeEach(() => {
    vi.resetModules();
  });

  afterEach(() => {
    if (originalAllowedDirectories !== undefined) {
      process.env.ALLOWED_DIRECTORIES = originalAllowedDirectories;
    } else {
      delete process.env.ALLOWED_DIRECTORIES;
    }
    if (originalDefaultWorkDirectory !== undefined) {
      process.env.DEFAULT_WORK_DIRECTORY = originalDefaultWorkDirectory;
    } else {
      delete process.env.DEFAULT_WORK_DIRECTORY;
    }
    if (originalEnableManualSessionBind !== undefined) {
      process.env.ENABLE_MANUAL_SESSION_BIND = originalEnableManualSessionBind;
    } else {
      delete process.env.ENABLE_MANUAL_SESSION_BIND;
    }
    if (originalProjectAliases !== undefined) {
      process.env.PROJECT_ALIASES = originalProjectAliases;
    } else {
      delete process.env.PROJECT_ALIASES;
    }
    vi.restoreAllMocks();
  });

  it('应包含 DEFAULT_WORK_DIRECTORY 与 ALLOWED_DIRECTORIES', async () => {
    const cwd = path.resolve(process.cwd());
    const parent = path.dirname(cwd);

    // 设置环境变量
    process.env.ALLOWED_DIRECTORIES = `${cwd},${parent}`;
    process.env.DEFAULT_WORK_DIRECTORY = parent;
    process.env.ENABLE_MANUAL_SESSION_BIND = 'false';
    process.env.PROJECT_ALIASES = JSON.stringify({
      '当前目录': cwd,
      '父目录': parent,
    });

    // 重新加载配置模块
    const { directoryConfig, userConfig } = await import('../src/config.js');

    // Mock chatSessionStore.getKnownDirectories 返回这些目录
    vi.spyOn(chatSessionStore, 'getKnownDirectories').mockReturnValue([cwd, parent]);

    const handler = new P2PHandler();
    const data = await (handler as unknown as InternalP2PHandler).buildCreateChatCardData();
    const directories = new Set((data.projectOptions || []).map(item => item.directory));

    expect(directories.has(cwd)).toBe(true);
    expect(directories.has(parent)).toBe(true);
  });

  it('应包含已存在会话目录', async () => {
    const cwd = path.resolve(process.cwd());
    const parent = path.dirname(cwd);
    const sessionDirectory = path.join(cwd, 'src');

    // 设置环境变量
    process.env.ALLOWED_DIRECTORIES = `${cwd},${parent}`;
    process.env.DEFAULT_WORK_DIRECTORY = parent;
    process.env.ENABLE_MANUAL_SESSION_BIND = 'true';

    // 重新加载配置模块
    const { directoryConfig, userConfig } = await import('../src/config.js');

    const opencodeSession = {
      id: 'ses_create_chat_1',
      title: '会话一',
      directory: sessionDirectory,
      time: {
        created: 0,
        updated: 0,
      },
    } as unknown as Awaited<ReturnType<typeof opencodeClient.listSessionsAcrossProjects>>[number];

    vi.spyOn(opencodeClient, 'listSessionsAcrossProjects').mockResolvedValue([opencodeSession]);
    vi.spyOn(chatSessionStore, 'getKnownDirectories').mockReturnValue([]);

    const handler = new P2PHandler();
    const data = await (handler as unknown as InternalP2PHandler).buildCreateChatCardData();
    const directories = new Set((data.projectOptions || []).map(item => item.directory));

    expect(directories.has(sessionDirectory)).toBe(true);
  });
});
